...
const Header = () => {
  ...
  return (
    <>
      <header className={`tiny:height-[6rem] tiny:!p-[0] !fixed lg:!p-10 z-50 w-full transition-all duration-500 ${isScrolled && location.pathname === "/" ? "bg-white shadow-2xl fixed" : isScrolled && location.pathname !== "/" ? "bg-white" : "bg-white shadow-2xl relative"} lg:py-4 h-[4.81em] lg:h-[2.688em] flex justify-center  `}>
        <div
          ...
          <NavLink to={"/"}>{location.pathname === "/" ? <img src={new_logo_white} alt="Quick Mynt" className="tiny:w-[8.2em] h-[2.51em] lg:w-[20.33em] lg:h-[3.78em] object-contain" /> : <img src={logo} alt="Quick Mynt" className="w-[6.2em] h-[2.51em] lg:w-[9.33em] lg:h-[3.78em] object-contain" />}</NavLink>
          ...
          <button
            className={` hidden ${isScrolled ? "bg-[#488559] border-2 border-[#488559]" : "bg-[#488559] border-2 border-[#488559]"}
                 text-[#ffffff] hover:text-[#488559] hover:bg-[#FFF] hover:border lg:flex justify-center items-center hover:border-[#488559] text-center rounded-full h-[2.5em] text-[1.31em] leading-[25.36px] font-[600] font-graphik text-[#FFF] lg:ml-[1rem] lg:w-[10rem]`}>
            Sign Up
          </button>

          {!showMbMenu && (
            <button className="scale-[1.5] bg-transparent lg:hidden p-0 border-none outline-none focus:border-none focus:outline-none" onClick={() => setShowMbMenu(true)}>
